...
#     app.run(host='0.0.0.0', port=port, debug=False) # Matikan debug=True saat production
...
# 1. Memuat File .env (Tetap dipertahankan untuk kebutuhan SECRET_KEY & GEMINI_API_KEY)
from dotenv import load_dotenv
load_dotenv()
...
# ---------------------------------------------------------------------------
# Health Check & Ping Routes (Steril Tanpa Mengakses Ekosistem Apapun)
# ---------------------------------------------------------------------------
@app.route("/health")
def health():
    return jsonify({
...
# Definisi Daftar Modul Utama (Tetap sesuai struktur asli aplikasi)
MODULES = [
    {"id": 0,  "title": "Introduction to Python",     "desc": "Langkah awal memahami sintaksis Python. Berkenalan dengan lingkungan ekosistem koding tanpa setup yang rumit."},
    {"id": 1,  "title": "Variables & Data Types",      "desc": "Fondasi dasar penyimpanan data di memori serta manipulasi nilai objek pertama kamu."},
    {"id": 2,  "title": "Conditionals (If-Else)",      "desc": "Mengatur alur pengambilan keputusan program berdasarkan logika dan kondisi tertentu."},
...
# ---------------------------------------------------------------------------
# PERBAIKAN: Mocking Context Processor Agar Template HTML Tidak Error/Pecah
# ---------------------------------------------------------------------------
@app.context_processor
def inject_user_progress_mock():
    # Mengirimkan array kosong dan nilai progress 0 sebagai pengganti query MySQL
    completed_dummy = []
    total_modules = len(MODULES)
    completed_count = len(completed_dummy)
    progress_percent = 0
    xp = 0
    return dict(
        modules=MODULES,
        completed_modules=completed_dummy,
        completed_count=completed_count,
        progress_percent=progress_percent,
        xp=xp,
    )
...
# ---------------------------------------------------------------------------
# Public & Authentication Routes (Dialihkan Menggunakan Session Sementara)
# ---------------------------------------------------------------------------
@app.route('/')
def landing_page(): 
    return render_template('pages/index.html')
...
def login_page():
    if request.method == 'POST':
        ...
        # BYPASS: Terima username apa saja tanpa mengecek hash password MySQL
        if username and password:
            session['username'] = username
            session['user_id'] = 999  # Dummy ID untuk session runtime
            return redirect(url_for('dashboard_page'))
        return render_template('pages/login.html', error='Username dan password wajib diisi!')
...
def register_page():
    if request.method == 'POST':
        # BYPASS: Langsung lempar ke halaman login dengan pesan sukses semu
        return render_template('pages/login.html', success='Registrasi berhasil (Simulasi/Database Off)! Silakan masuk.')
...
# ---------------------------------------------------------------------------
# API Endpoints (Bypass Langsung Memanggil File Statis modules_content.json)
# ---------------------------------------------------------------------------
@app.route('/api/module/<int:module_id>/get_quiz', methods=['GET'])
def get_or_generate_quiz(module_id):
    """Mengambil kuis langsung dari berkas JSON lokal tanpa menyentuh database atau AI."""
    if 'username' not in session:
        return jsonify({'success': False, 'error': 'Unauthorized'}), 401
    
    str_module_id = str(module_id)
    quiz_data = MODULES_CONTENT.get(str_module_id, {}).get('quiz', {})
...
def verify_user_code(module_id):
    ...
    starter_code = fallback_quiz.get('starter_code', '')
    expected_output = fallback_quiz.get('expected_output', '')
    module_title = MODULES[module_id]['title']

    try:
        # Evaluasi AI Gemini tetap kita pertahankan jalurnya untuk uji coba API Key cloud
        ai_response_raw = verify_code(module_title, quiz_title, starter_code, expected_output, user_code)
        evaluation_data = json.loads(ai_response_raw)
        ...
        print(f"[API Verify Error Isolated]: {str(e)}")
        return jsonify({'success': False, 'error': 'Gagal memproses evaluasi kode oleh AI.'}), 500

@app.route('/api/module/<int:module_id>/complete', methods=['POST'])
def complete_module(module_id):
    """Bypass penyimpanan progres kuis ke database."""
    if 'username' not in session: 
        return jsonify({'success': False, 'error': 'Not authenticated'}), 401
    
    return jsonify({'success': True, 'completed_modules': [module_id], 'xp': 100})
